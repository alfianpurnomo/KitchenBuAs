...
class Product extends CI_Controller {
    ...
    public function add() {
        ...
        $this->data['categorys'] = $category_product;
            ...
            if ($this->validateForm()) {
                ...
                if ($post_image['thumbnail_image']['tmp_name']) {
                    ...
                    try {
	                    ...
	                    // update data
                        $img_arr = array(
							'thumbnail_image' => $this->class_path_name . '/' . $picture_db
						);
				...
				if($post['is_twitter']) {
					...
					$parameters = [
    						'status' => $post['title'].' '.base_url().$category['slug'].'/'.$id.'/'.$post['uri_path'].'#twitter',
					...
					//$post_status = $tw->post('statuses/update', ['status'=>$post['title'].' '.base_url().$category['slug'].'/'.$id.'/'.$post['uri_path'].'#twitter']);
